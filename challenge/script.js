...
function applyPlayerSkins(player, gear) {
    ...
    if (gear.gridSkin && gear.gridSkin !== 'grid_default') {
        const skinName = gear.gridSkin.replace('grid_', '');
...
class Player {
    ...
    displaySequence() {
        ...
        const skinName = arrowSkin ? arrowSkin.replace('arrow_', '') : 'default';
        const arrowSkinClass = (skinName && skinName !== 'default') ? `arrow-skin-${skinName}` : '';

        sequence.forEach((arrowKey, index) => {
            // Add a directional class for CSS targeting (e.g., 'arrow-up')
            const directionClass = `arrow-${arrowKey.replace('Arrow', '').toLowerCase()}`;
            // Add the arrow skin class AND the direction class to the SVG string
            let arrowHTML = ARROW_SVG[arrowKey].replace('class="arrow-icon"', `class="arrow-icon ${arrowSkinClass} ${directionClass}"`);
