...
function setupUIListeners() {
    ...
    // Tab Listeners
    DOMElements.welcomeTab.addEventListener('click', () => {
        DOMElements.welcomeTab.classList.add('active');
        DOMElements.statsTab.classList.remove('active');
        DOMElements.welcomeContent.classList.remove('hidden');
        DOMElements.statsContent.classList.add('hidden');
    });
    DOMElements.statsTab.addEventListener('click', () => {
        updateStatsTab(); // Update stats when tab is clicked
        ...
        DOMElements.welcomeContent.classList.add('hidden');
    });
...
function restartMatch() {
    ...
    DOMElements.rematchBtn.textContent = "Rematch";
...
function generatePowerUps() {
    const allPowerUps = [
        ...
        { 
            title: 'Combo Shield', 
            desc: 'Your combo won\'t reset on your next mistake.',
            badge: '🛡️',
            effect: () => { localPlayer.hasComboShield = true; }
        },
        { 
            title: 'Overclock', 
            desc: 'Adds 60 seconds to the next stage\'s timer.',
            badge: '⏳',
...
function resetGameToHub() {
    ...
    // Re-initialize lobby connection
    connectToLobby();
}
