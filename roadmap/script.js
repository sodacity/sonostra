...
// --- BUG FIX: MAP SHRINKING ---
/**
 * Toggles the visibility of the main hub panel (news, stats, etc.).
 * Instead of using display:none, it uses opacity and pointer-events
 * to prevent the parent container from collapsing, which would shrink the map.
...
function toggleHubPanel(show) {
    ...
    const toggleBtn = DOMElements.toggleHubBtn;

    if (show) {
        newsContainer.classList.remove('hidden');
        newsContainer.style.opacity = '1';
        newsContainer.style.pointerEvents = 'auto';
        newsContainer.classList.remove('anim-pop-out');
...
function setupUIListeners() {
    ...
    DOMElements.hubPlayGameBtn.addEventListener('click', () => {
        // This button is inside the panel, so we hide the panel.
    ...
    DOMElements.closeSetupBtn.addEventListener('click', () => {
        ...
        // If the news container is not visible, show the toggle button
