...
class Player {
    ...
    startNewSequence(sequence = null) {
        ...
        if (this.isLocal && gameState.mode === 'multi' && !sequence) {
    ...
    conn.on('data', async (data) => {
        switch(data.type) {
            ...
            case 'video_change':
                ...
                break;
            case 'new_sequence':
                 if (localPlayer) localPlayer.startNewSequence(data.sequence);
                break;
                ...
                if (remotePlayer) {
                    ...
                    if (gameState.isHost && remotePlayer.isIdle && gameState.pendingNextWave) {
                        ...
                        if (gameState.isCrescendoWave) {
                            ...
                            const nextWaveNumber = gameState.currentWave + 1;
                            if (gameState.isHost && gameState.mode === 'multi') {
                                sendData({ type: 'wave_announcement_trigger', wave: nextWaveNumber });
                            }
                            (async () => {
                                await showAnnouncement(DOMElements.waveAnnouncement, `WAVE ${nextWaveNumber}`, 2000);
            ...
            case 'boss_attack':
                ...
                break;
            case 'damage_boss':
                if(gameState.isHost) dealDamageToBoss(10, remotePlayer, false);
                break;
            case 'wave_change':
                if (!gameState.isHost) {
            ...
            case 'show_intermission':
                ...
                break;
            case 'wave_announcement_trigger':
                await showAnnouncement(DOMElements.waveAnnouncement, `WAVE ${data.wave}`, 2000);
                break;
        }
    });
    conn.on('close', () => {
...
async function startWave(waveNumber, bossData = null) {
    ...
    } else if (waveNumber === 1 && gameState.bossMode) {
    ...
    const allPowerUps = [
        {
            ...
            effect: () => {
                ...
                gameState.enrageThreshold = Math.min(0.75, gameState.enrageThreshold + 0.15);
            }
        },
        {
            title: 'Sequence Flow',
