    ...
    conn.on('data', (data) => {
        ...
        switch(data.type) {
            ...
            case 'damage_boss':
                ...
                break;
            // --- BUG FIX 1: STAT TRACKING ---
            // Client receives confirmation of damage dealt from the host
            case 'confirm_damage':
                if (localPlayer) {
...
async function dealDamageToBoss(damage, player, isHeavyAttack = false) {
    ...
    // --- BUG FIX 1: STAT TRACKING ---
    // If the player who dealt damage is the remote client, send their new total back
    ...
    if (newHealth <= 0) {
        ...
        // --- BUG FIX 2: FINAL BLOW INTERRUPTION ---
        // If the remote player landed the final blow while the host (localPlayer) is busy
        if (!player.isLocal && !localPlayer.isIdle) {
            gameState.pendingNextWave = true;
            // Play the death animation, but don't proceed to the next wave yet.
            playDeathAnimation(gameState.isCrescendoWave);
            return; // Exit the function and wait for the host to finish.
        }
        // --- END BUG FIX ---
...
function handleCorrectKeyPress(player) {
    ...
    if (player.sequenceProgress === player.currentSequence.length) {
        ...
        // --- BUG FIX 2: FINAL BLOW INTERRUPTION ---
        // If this is the host, and a wave transition was pending, trigger it now.
        if (gameState.isHost && player.isLocal && gameState.pendingNextWave) {
            gameState.pendingNextWave = false; // Reset the flag
            // Now, proceed to the next wave/intermission
            if (gameState.isCrescendoWave) {
                clearInterval(gameState.waveModeTimerId);
                showIntermissionScreen();
