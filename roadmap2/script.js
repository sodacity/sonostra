...
function setupUIListeners() {
    ...
    // The listener for the GEAR button is in gear.js, but we'll add a placeholder here
    // in case gear.js doesn't load.
    if (DOMElements.toggleGearBtn) {
        DOMElements.toggleGearBtn.addEventListener('click', () => {
            if (typeof initGearSystem === 'function') {
                const gearModal = document.getElementById('gear-modal');
                updateGearModal();
                gearModal.classList.remove('hidden');
            } else {
                console.warn("Gear system not loaded.");
            }
        });
    }
