...
const defaultSettings = {
    ...
    musicMuted: false,
    gridSkin: 'grid_default',
    arrowSkin: 'arrow_default',
    particleEffect: 'particle_default'
};
...
function applySettings() {
    ...
    if (localPlayer) {
        ...
        // Also update the gear object for the local player
        localPlayer.gear.gridSkin = currentSettings.gridSkin;
        localPlayer.gear.arrowSkin = currentSettings.arrowSkin;
        localPlayer.gear.particleEffect = currentSettings.particleEffect;
    }
...
const ARROW_SKIN_DATA = {
    ...
    'arrow_inset': {
        'ArrowUp': `<svg class="arrow-icon" viewBox="0 0 100 100"><path class="arrow-bg" d="M0 20 Q 0 0 20 0 L 80 0 Q 100 0 100 20 L 100 80 Q 100 100 80 100 L 20 100 Q 0 100 0 80 Z"></path><path class="arrow-shape" d="M50 25 L70 50 L58 50 L58 75 L42 75 L42 50 L30 50 Z"></path></svg>`,
        'ArrowDown': `<svg class="arrow-icon" viewBox="0 0 100 100"><path class="arrow-bg" d="M0 20 Q 0 0 20 0 L 80 0 Q 100 0 100 20 L 100 80 Q 100 100 80 100 L 20 100 Q 0 100 0 80 Z"></path><path class="arrow-shape" d="M50 75 L70 50 L58 50 L58 25 L42 25 L42 50 L30 50 Z"></path></svg>`,
        'ArrowLeft': `<svg class="arrow-icon" viewBox="0 0 100 100"><path class="arrow-bg" d="M0 20 Q 0 0 20 0 L 80 0 Q 100 0 100 20 L 100 80 Q 100 100 80 100 L 20 100 Q 0 100 0 80 Z"></path><path class="arrow-shape" d="M25 50 L50 30 L50 42 L75 42 L75 58 L50 58 L50 70 Z"></path></svg>`,
        'ArrowRight': `<svg class="arrow-icon" viewBox="0 0 100 100"><path class="arrow-bg" d="M0 20 Q 0 0 20 0 L 80 0 Q 100 0 100 20 L 100 80 Q 100 100 80 100 L 20 100 Q 0 100 0 80 Z"></path><path class="arrow-shape" d="M75 50 L50 30 L50 42 L25 42 L25 58 L50 58 L50 70 Z"></path></svg>`
    },
    // --- FIX: Added the SVG data for the new "Inverted" skin ---
    'arrow_inverted': {
        'ArrowUp': `<svg class="arrow-icon" viewBox="0 0 100 100"><path class="arrow-bg" d="M0 20 Q 0 0 20 0 L 80 0 Q 100 0 100 20 L 100 80 Q 100 100 80 100 L 20 100 Q 0 100 0 80 Z"></path><path class="arrow-shape" d="M50 25 L70 50 L58 50 L58 75 L42 75 L42 50 L30 50 Z"></path></svg>`,
        'ArrowDown': `<svg class="arrow-icon" viewBox="0 0 100 100"><path class="arrow-bg" d="M0 20 Q 0 0 20 0 L 80 0 Q 100 0 100 20 L 100 80 Q 100 100 80 100 L 20 100 Q 0 100 0 80 Z"></path><path class="arrow-shape" d="M50 75 L70 50 L58 50 L58 25 L42 25 L42 50 L30 50 Z"></path></svg>`,
        'ArrowLeft': `<svg class="arrow-icon" viewBox="0 0 100 100"><path class="arrow-bg" d="M0 20 Q 0 0 20 0 L 80 0 Q 100 0 100 20 L 100 80 Q 100 100 80 100 L 20 100 Q 0 100 0 80 Z"></path><path class="arrow-shape" d="M25 50 L50 30 L50 42 L75 42 L75 58 L50 58 L50 70 Z"></path></svg>`,
...
// --- NEW: PARTICLE EFFECT LOGIC ---
/**
 * Triggers a particle effect on a player's grid.
 * @param {Player} player The player object (local or remote).
 * @param {string} arrowKey The direction of the arrow hit, for color.
 */
function triggerHitParticles(player, arrowKey) {
    // *** MODIFIED: Use the player's specific gear, not global settings ***
    const effect = player.gear.particleEffect || 'particle_default';
    if (effect === 'particle_default') return;

    const grid = player.dom.grid;
    const arrowColor = getArrowColor(arrowKey);
    ...
    switch (effect) {
        case 'particle_burst':
            for (let i = 0; i < 8; i++) {
                ...
                p.style.background = arrowColor;
                p.style.boxShadow = `0 0 8px ${arrowColor}`;
                grid.appendChild(p);
                setTimeout(() => p.remove(), 600);
            }
            break;
        case 'particle_shards':
            for (let i = 0; i < 10; i++) {
                const p = document.createElement('div');
                p.className = 'particle shard';
...
/**
 * Helper to get the base color for a given arrow direction.
 * @param {string} arrowKey 
 * @returns {string} The hex color code.
 */
function getArrowColor(arrowKey) {
    switch(arrowKey) {
        case 'ArrowUp': return '#03A9F4';
        case 'ArrowDown': return '#F44336';
        case 'ArrowLeft': return '#FF00FF';
        case 'ArrowRight': return '#4CAF50';
...
class Player {
    ...
    displaySequence() {
        ...
        // *** MODIFIED: Always get skin from the player's own gear object ***
        const arrowSkin = this.gear.arrowSkin || 'arrow_default';
        const skinName = arrowSkin ? arrowSkin.replace('arrow_', '').replace(/_/g, '-') : 'default';
        const arrowSkinClass = (skinName && skinName !== 'default') ? `arrow-skin-${skinName}` : '';
    
        sequence.forEach((arrowKey, index) => {
            const arrowSVGString = ARROW_SKIN_DATA[arrowSkin] ? ARROW_SKIN_DATA[arrowSkin][arrowKey] : ARROW_SKIN_DATA['arrow_default'][arrowKey];
    
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = arrowSVGString;
            const svgElement = tempDiv.querySelector('.arrow-icon');
            
            if (arrowSkinClass) {
                svgElement.classList.add(arrowSkinClass);
            }
        
            const direction = arrowKey.replace('Arrow', '').toLowerCase();
            svgElement.classList.add(`arrow-${direction}`);
        
            if (this.isLocal && this.hasVulnerabilityScan && index === 0) {
                const bgElement = tempDiv.querySelector('.arrow-bg');
                if (bgElement) bgElement.classList.add('critical'); 
            }
...
function populateHowToPlay() {
    ...
    if(exampleArrowsContainer) {
        ...
        ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].forEach(arrow => {
            exampleArrowsContainer.innerHTML += ARROW_SKIN_DATA['arrow_default'][arrow];
        });
    }
    ...
    if(exampleSequenceContainer) {
        ...
        ['ArrowRight', 'ArrowUp', 'ArrowUp', 'ArrowLeft'].forEach(arrow => {
            exampleSequenceContainer.innerHTML += ARROW_SKIN_DATA['arrow_default'][arrow];
        });
    }
}
...
function createHubGlyphs() {
    ...
    const glyphCount = 30;
    const arrowKeys = Object.keys(ARROW_SKIN_DATA['arrow_default']);
    ...
    for (let i = 0; i < glyphCount; i++) {
        ...
        glyph.innerHTML = ARROW_SKIN_DATA['arrow_default'][randomArrowKey];
...
function onStartGameClick() {
    ...
    if (gameState.mode === 'solo') {
        ...
        peer.on('open', (id) => {
            ...
            // --- FIX: Immediately update host's gear from settings ---
            localPlayer.gear.gridSkin = currentSettings.gridSkin;
            localPlayer.gear.arrowSkin = currentSettings.arrowSkin;
            localPlayer.gear.particleEffect = currentSettings.particleEffect;
            // ---------------------------------------------------------
            ...
            peer.on('connection', (newConn) => {
                ...
                conn.on('open', () => {
                    ...
                    remotePlayer.gear = remoteData.gear; // FIX #1a: Save guest's gear
                    ...
                    conn.send({
                        ...
                        hostData: { 
                            ...
                            gear: localPlayer.gear // FIX #1b: Send host's gear
                        },
        ...
        conn = peer.connect(roomName, {
            metadata: { 
                ...
                gear: {
                    gridSkin: currentSettings.gridSkin,
                    arrowSkin: currentSettings.arrowSkin,
                    particleEffect: currentSettings.particleEffect
                }
            }
        });
    ...
    conn.on('data', (data) => {
        ...
        switch(data.type) {
             case 'connection_success': // Only client receives this
                ...
                remotePlayer.gear = data.hostData.gear; // FIX #1c: Save host's gear
            ...
            case 'correct_hit':
                if (remotePlayer) {
                    triggerHitParticles(remotePlayer, data.arrowKey);
                    const arrowEl = remotePlayer.dom.sequenceContainer.children[data.progressIndex];
...
function displaySequenceStaggered(player) {
    ...
    sequence.forEach((arrowKey, index) => {
        setTimeout(() => {
            const arrowSkin = player.gear.arrowSkin || 'arrow_default';
            const skinName = arrowSkin ? arrowSkin.replace('arrow_', '').replace(/_/g, '-') : 'default';
            const arrowSkinClass = (skinName && skinName !== 'default') ? `arrow-skin-${skinName}` : '';
...
async function dealDamageToBoss(damage, player, isHeavyAttack = false) {
    ...
    if (newHealth <= 0) {
        ...
        // FIX #2: Add a delay before starting the next sequence
        setTimeout(() => {
...
function handleCorrectKeyPress(player) {
    const arrowKey = player.currentSequence[player.sequenceProgress];
    triggerHitParticles(player, arrowKey);

    // *** MODIFIED: Send hit data to opponent ***
    if (player.isLocal && gameState.mode === 'multi') {
        sendData({ type: 'correct_hit', arrowKey: arrowKey, progressIndex: player.sequenceProgress });
    }
    ...
    if (player.sequenceProgress === player.currentSequence.length) {
        ...
        if (gameState.bossMode) {
            ...
            setTimeout(() => player.startNewSequence(), 500);
        }
    }
}
